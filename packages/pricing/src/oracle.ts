...
export class ReflectorFiatOracle implements FiatRateSource {
  ...
  constructor(private readonly config: { rpcUrl: string; networkPassphrase: string; contractId: string } = REFLECTOR_FIAT.mainnet) {
  ...
  async getRate(currency: string): Promise<FiatRate> {
    ...
    const record = (await this.call("lastprice", asset)) as { price: bigint; timestamp: bigint } | undefined;
    if (!record) {
      throw new Error(`Reflector oracle ${this.config.contractId} has no rate for ${currency}`);
    }
    return {
      usdPerUnit: record.price,
      decimals,
